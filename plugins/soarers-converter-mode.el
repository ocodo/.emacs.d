(defvar soarers-converter-basic-offset 2)

(defun soarers-converter-indent-line ()
  ...
  (if (bobp)  ; Check for rule 1
    ...
    (let ((not-indented t) cur-indent)
      (if (looking-at "^[ \t]*endblock") ; Check for rule 2
          (progn
            (save-excursion
              (forward-line -1)
              (setq cur-indent (- (current-indentation) soarers-converter-basic-offset)))
            (if (< cur-indent 0)
                (setq cur-indent 0)))
        (save-excursion
          (while not-indented
            ...
            (if (looking-at "^[ \t]*\(?:end\(?:block\|macro\)\)") ; Check for rule 3
                (progn
                  (setq cur-indent (current-indentation))
                  (setq not-indented nil))
                                        ; Check for rule 4
              (if (looking-at (concat "^[ \t]*" (regexp-opt soarers-converter-hid-codes)))
                  (progn
                    (setq cur-indent (+ (current-indentation) soarers-converter-basic-offset))
                    (setq not-indented nil))
...
(ignore-errors
  ...
  (defmacro soarers-converter-mode-should-indent (from to)
...
(defconst soarers-converter-commands
  '("include"
    "force"
    "ifselect"
    "ifset"
...
(defconst soarers-converter-hid-codes
...
(define-derived-mode soarers-converter-mode
  ...
  (setq indent-line-function 'soarers-converter-indent-line)
  (setq font-lock-defaults soarers-converter-font-lock-defaults))
